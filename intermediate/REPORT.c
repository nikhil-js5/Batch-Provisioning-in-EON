...
int main() {
    ...
    //std::srand(static_cast<unsigned int>(std::time(nullptr))); // time used as seed to get random sequence each time
    std::srand(99); // fixed seed value
    ...
    cout << "Number of requests to be generated: ";
    cin >> numReq;
    ...
    cout << "\n\nGenerated Network Connections:" << endl;
    for (int i = 0; i < numNodes; ++i) {
        for (int j = i + 1; j < numNodes; ++j) {
            if (network[i][j] == 1) {
    ...
    for (int i = 0; i < numSets; ++i) {
        ...
        for (const auto& request : sets[i]) {
            cout << "ID: " << request.getId() << ", Source : " << request.getSourceNode()
                 ...
                 << ", Number of Hops: " << request.getNumHops()
                 << endl;
        }
        cout << endl;
    } // */
    ...
    cout << "\n\nNumber of elements in superSet (total no of requests) = " << supSetCount <<"\n\n" <<endl;

    int batchCapacity = 30;
    ...
    for (int i = 0; i < batches.size(); ++i) {
        ...
        for (const auto& request : batches[i]) {
            std::cout << "ID: " << request.getId() << ", Source: " << request.getSourceNode()
                 << ", Destination: " << request.getDestinationNode() << ", Slots Req: " << request.getSlotsRequired()
                 << ", No. of Hops: " << request.getNumHops()  << std::endl;
        }
        std::cout << std::endl;
    }
