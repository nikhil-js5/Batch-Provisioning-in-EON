...
int main() {
    ...
    for (int i = 0; i < numNodes; ++i) {
        for (int j = i + 1; j < numNodes; ++j) {
            /*// Generate a random number (0 or 1) to determine if there's a connection*/
            //int isConnected = rand() % 2;
            int isConnected;
            if ((std::rand() % 1000 + 1) <= 20) {
                isConnected = 1;
            } else {
                isConnected = 0;
            }

            // int isConnected = 1; // to generate a completely connected network
            if (isConnected) {
                network[i][j] = 1;
                network[j][i] = 1;
                numConnections++;
    ...
    cout << "SuperSetFormed : Enter the number of requests to be generated: ";
    cin >> numReq;
